...
class Robot:
    ...
    * visuals: the list of all the 'visual' 3D objects to render the robot, each element of the list being
    ...
    def createArm3DOF(self, rootId=0, prefix="", jointPlacement=SE3.Identity()):
        ...
        self.viewer.viewer.gui.addSphere("world/" + prefix + "hip", 0.1, colorred)
        self.visuals.append(Visual("world/" + prefix + "hip", jointId, SE3.Identity()))
        self.viewer.viewer.gui.addBox(
            "world/" + prefix + "upperbody", 0.1, 0.1, 2.9, color
        )
        self.visuals.append(
            Visual(
                "world/" + prefix + "upperbody",
                jointId,
                SE3(eye(3), np.array([0.0, 0.0, 1.5])),
            )
        )
        ...
        jointId = self.model.addJoint(jointId, joint, jointPlacement, jointName)
        self.model.appendBodyToJoint(jointId, Inertia.Random(), SE3.Identity())
        self.viewer.viewer.gui.addSphere("world/" + prefix + "shoulder", 0.1, colorred)
        self.visuals.append(
            Visual("world/" + prefix + "shoulder", jointId, SE3.Identity())
        )
        self.viewer.viewer.gui.addBox(
            "world/" + prefix + "upperarm", 0.1, 0.1, 0.9, color
        )
        self.visuals.append(
            Visual(
                "world/" + prefix + "upperarm",
                jointId,
                SE3(eye(3), np.array([0.0, 0.0, 0.5])),
            )
        )
        ...
        jointId = self.model.addJoint(jointId, joint, jointPlacement, jointName)
        self.model.appendBodyToJoint(jointId, Inertia.Random(), SE3.Identity())
        self.viewer.viewer.gui.addSphere("world/" + prefix + "elbow", 0.1, colorred)
        self.visuals.append(
            Visual("world/" + prefix + "elbow", jointId, SE3.Identity())
        )
        self.viewer.viewer.gui.addBox(
            "world/" + prefix + "lowerarm", 0.1, 0.1, 0.9, color
        )
        self.visuasls.append(
            Visual(
                "world/" + prefix + "lowerarm",
                jointId,
                SE3(eye(3), np.array([0.0, 0.0, 0.5])),
            )
        )
        ...
        jointId = self.model.addJoint(jointId, joint, jointPlacement, jointName)
        self.model.appendBodyToJoint(jointId, Inertia.Random(), SE3.Identity())
        self.viewer.viewer.gui.addSphere("world/" + prefix + "wrist", 0.1, colorred)
        self.visuals.append(
            Visual("world/" + prefix + "wrist", jointId, SE3.Identity())
        )
        self.viewer.viewer.gui.addBox(
            "world/" + prefix + "atlatl", 0.1, 0.1, 0.9, color
        )
        self.visuals.append(
            Visual(
                "world/" + prefix + "atlatl",
        ...
        jointName = prefix + "P_joint"
        jointPlacement = SE3(eye(3), np.array([0, 0, 1.0]))
        joint = JointModelRX()
        jointId = self.model.addJoint(jointId, joint, jointPlacement, jointName)
        self.model.appendBodyToJoint(jointId, Inertia.Random(), SE3.Identity())
        self.viewer.viewer.gui.addSphere("world/" + prefix + "P", 0.1, colorred)
        self.visuals.append(Visual("world/" + prefix + "P", jointId, SE3.Identity()))
        self.viewer.viewer.gui.addBox("world/" + prefix + "dart", 0.1, 0.1, 0.25, color)
        self.visuals.append(
            Visual(
                "world/" + prefix + "dart",
                jointId,
                SE3(eye(3), np.array([0.0, 0.0, 0.15])),
            )
        )
